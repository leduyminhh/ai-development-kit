...
$script = Join-Path $Root 'adapters/codex/hooks/log-agent-event.ps1'
...
'@
    ...
    $line = $rows[0]
    Assert-True ($line.StartsWith('2026-04-15T07:00:00+07:00 [INFO] [ai-engineering-platform] [java-analyze] [trace-111] codex.project.agent - Review payment flow | ')) 'Text log prefix mismatch.'
    Assert-True ($line.Contains('schema=codex.project.event.v1')) 'Schema field mismatch.'
    Assert-True ($line.Contains('service=ai-engineering-platform')) 'Service field mismatch.'
