...
### State and ownership
All managed state lives in a single flat file: `<scope-root>/.ai-engineering/manifest.json`. It records every installed file path and the plugins/providers that own them. `cli/lib/install.mjs` uses this manifest to distinguish platform-managed files (safe to overwrite/remove) from user-owned content (must be preserved). Managed instruction files (`AGENTS.md`, `CLAUDE.md`) are updated *in place*: `cli/lib/managed-block.mjs` rewrites only the "AI Engineering" baseline block between its markers, leaving all content outside them untouched. Files are written via `cli/lib/write.mjs`, which attempts a symlink and falls back to a direct copy on platforms where symlink creation fails (e.g. Windows without Developer Mode).
...
### Interactive wizards
`cli/lib/wizard.mjs` implements a step-machine that drives all interactive flows (install, uninstall, build, check). Zero-dep TUI primitives (`selectOne`, `selectMany`, `confirmStep`) live in `cli/lib/prompt.mjs`. Step logic is separated from terminal I/O so it is unit-testable without a TTY. `cli/index.mjs` is the entry point: it parses args, then routes to the wizard (when stdin is a TTY and no explicit flags are given) or directly to non-interactive execution (`--yes` + explicit `--provider`/`--plugin`). `AIE_INSTALL_ROOT` overrides the scope root, used by tests to isolate installs.
