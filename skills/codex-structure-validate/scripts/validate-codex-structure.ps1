...
function Get-DefaultManifestText {
    param([string[]]$SkillNames)

    $header = @'
# Explicit contract between repo skill assets, runtime registration, and external Codex discovery.
...
[external_discovery]
loader_root = "~/.codex/skills"
install_script = "scripts/install-skill-link.ps1"
link_name = "codex-workflow-kit"
linked_source = "skills"
'@

    $entries = foreach ($skillName in ($SkillNames | Sort-Object -Unique)) {
@"

[[skill]]
name = "$skillName"
skill_path = "skills/$skillName/SKILL.md"
ui_metadata = "skills/$skillName/metadata/openai.yaml"
agent_entry = ""
registered_agent = ""
kind = "unmapped"
"@
    ...
    return ($header.TrimEnd() + ($entries -join ''))
}
...
$skillsManifestPath = Join-Path $skillsRoot 'manifest.toml'
Ensure-ScaffoldDirectory -Path $codexRoot -Label 'Codex root' -TrackWhenEmpty $false
Ensure-ScaffoldDirectory -Path $skillsRoot -Label 'Skills root' -TrackWhenEmpty $false
...
if (Test-Path -LiteralPath $skillsRoot) {
    if (Test-Path -LiteralPath $skillsManifestPath) {
        $manifestText = Get-Content -LiteralPath $skillsManifestPath -Raw
        $findings.Add((New-Finding 'pass' "Skills manifest exists: $skillsManifestPath"))
        foreach ($skillName in $skillNames) {
            if ($manifestText -match "(?m)^name\s*=\s*""$([regex]::Escape($skillName))""\s*$") {
                $findings.Add((New-Finding 'pass' "Skills manifest includes: $skillName"))
            } else {
                $findings.Add((New-Finding 'fail' "skills/manifest.toml must include skill entry: $skillName"))
            }
        }
    } elseif ($Fix) {
        $defaultManifest = Get-DefaultManifestText -SkillNames @($skillNames)
        Set-Content -LiteralPath $skillsManifestPath -Value $defaultManifest -Encoding utf8
        $findings.Add((New-Finding 'pass' "Skills manifest scaffold created: $skillsManifestPath"))
    } else {
        $findings.Add((New-Finding 'fail' 'skills/manifest.toml is missing. Add it to define the skill-to-agent contract.'))
    }
}
