...
Use this skill when building or modifying React UI from Figma designs, tickets, text requirements, API examples such as curl, or when integrating APIs into React application flows.
...
- Use `resources/api-integration-from-curl.md` when the user provides curl or API examples.
- Use `resources/figma-ui-implementation.md` when implementing from Figma.
- Use `react-accessibility-review` before claiming a user-facing component is ready.

## Common Rationalizations

| Rationalization | Rebuttal |
|---|---|
| "The visual is close enough." | UI work must preserve usability, responsive behavior, and accessibility. |
| "Mock data is fine for now." | Mock data must be explicit and should not hide missing integration requirements. |
| "One viewport screenshot is enough." | Responsive UI needs desktop and mobile verification when layout changes. |

## Red Flags

- Text overflows buttons, cards, or compact panels.
- UI creates a landing page instead of the requested working experience.
- API errors, loading states, or empty states are missing.
- Styling ignores existing app conventions.
