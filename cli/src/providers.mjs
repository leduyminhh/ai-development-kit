...
const projectorEntries = await Promise.all(
...
function legacyProjection(plan, input) {
  ...
  const workflowAsset = plan.assets.find(
    (asset) => asset.assetType === "command-catalog",
  );
  return {
    manifest: JSON.parse(
      plan.assets.find((asset) => asset.assetType === "provider-manifest")
        ?.content ?? "{}",
    ),
...
export function projectProviders(input) {
  if (Array.isArray(input)) {
    return Object.fromEntries(
      input.map((item) => [item.provider, projectProvider(item)]),
    );
  }
  return {
