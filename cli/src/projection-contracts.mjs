...
export function validateProjectionPlan(plan) {
  ...
  for (const asset of plan.assets ?? []) {
    if (!OPERATIONS.has(asset.operation)) {
      throw projectionError(
        `projection ${plan.provider}/${plan.scope} has invalid operation ${asset.operation}`,
      );
    }
    asset.destinationPath = assertContainedRelativePath(
      asset.destinationPath,
      `projection ${plan.provider}/${plan.scope}`,
    );
    if (destinations.has(asset.destinationPath)) {
      throw projectionError(
        `projection ${plan.provider}/${plan.scope} contains duplicate destination ${asset.destinationPath}`,
      );
    }
    ...
    if (asset.operation === "copy") {
      assertContainedRelativePath(
        asset.sourcePath,
        `projection ${plan.provider}/${plan.scope} source`,
      );
    ...
    if (
      !Array.isArray(asset.owners) ||
      asset.owners.length === 0 ||
      JSON.stringify(asset.owners) !==
        JSON.stringify([...new Set(asset.owners)].sort())
    ) {
      throw projectionError(
        `projection asset ${asset.assetId} owners must be sorted and unique`,
      );
    }
    if (typeof asset.shared !== "boolean") {
      throw projectionError(
        `projection asset ${asset.assetId} shared must be boolean`,
      );
    }
  }
  for (const instruction of plan.instructions ?? []) {
    assertContainedRelativePath(
      ...
      `projection ${plan.provider}/${plan.scope} instruction`,
    );
    assertContainedRelativePath(
      instruction.templatePath,
      `projection ${plan.provider}/${plan.scope} instruction template`,
    );
  }
  if (plan.mcpConfig) {
    assertContainedRelativePath(
      plan.mcpConfig.destinationPath,
      `projection ${plan.provider}/${plan.scope} MCP config`,
    );
  }
  return plan;
}
