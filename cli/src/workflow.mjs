...
function validateStepIds(steps) {
  ...
  for (const step of steps) {
    if (!step.id) return "step missing required id";
...
function detectCycle(steps) {
  ...
  for (const step of steps) {
    if (dfs(step.id)) return `dependency cycle detected involving step: ${step.id}`;
  }
  return null;
}
...
function topologicalSort(steps) {
  ...
  function visit(stepId) {
    ...
    if (!step) return;
    for (const dep of (step.depends || [])) { visit(dep); }
    result.push(stepId);
  }
...
function generateStepInstructions(wf, step) {
  ...
  if (step.depends && step.depends.length) lines.push("- Dependencies: " + step.depends.join(", "));
  if (step.input) {
    lines.push("- Input:");
  ...
  if (step.gates && step.gates.length) lines.push("- Gates: " + step.gates.join(", "));
  if (step.onError) {
    lines.push("- Error policy: " + (step.onError.policy || "stop"));
    if (step.onError.retryDelay) lines.push("  - Retry delay: " + step.onError.retryDelay);
    if (step.onError.fallbackStep) lines.push("  - Fallback: " + step.onError.fallbackStep);
  }
  return lines.join("\n");
}
...
function validateWorkflowDefinition(wf) {
  const errors = [];
  if (!wf || typeof wf !== "object") return ["workflow definition is empty"];

  if (!wf.id || typeof wf.id !== "string") errors.push("missing or invalid: id");
  if (!wf.description) errors.push("missing required field: description");
  if (!wf.version) errors.push("missing required field: version");
...
function generateInstructions(wf) {
  ...
  for (let i = 0; i < wf.steps.length; i++) {
    ...
    if (s.depends?.length) lines.push(`- Dependencies: ${s.depends.join(", ")}`);
    if (s.input) {
      for (const [k, v] of Object.entries(s.input)) {
        lines.push(`- Input "${k}": ${v}`);
      }
    }
    if (s.gates?.length) lines.push(`- Gates: ${s.gates.join(", ")}`);
    if (s.onError) lines.push(`- On error: ${s.onError.policy}`);
    lines.push("");
  }
  return lines.join("\n");
}
  ...
  if (await fileExists(defsRoot)) {
    for (const f of (await readdir(defsRoot)).filter((x) => x.endsWith(".yaml"))) {
      try {
        ...
        for (const e of validateWorkflowDefinition(wf)) errors.push(`${f.replace(/\.yaml$/, "")}: ${e}`);
      } catch (e) {
    ...
    if (await fileExists(coreRoot)) {
      for (const f of (await readdir(coreRoot)).filter((x) => x.endsWith(".yaml"))) {
        try {
          ...
          for (const e of validateWorkflowDefinition(wf)) errors.push(`core/${wfId}: ${e}`);
        } catch (e) {
...
export async function workflowBuild({ target, core, workflowId }) {
  ...
  if (errs.length > 0) return { status: "fail", workflowId, errors: errs, instructions: null };
...
export async function workflowStepNext({ target, workflowId, runId }) {
  ...
  if (state.status !== "running") return { status: state.status, workflowId, runId };
...
export async function workflowStatus({ target, workflowId, runId }) {
  ...
  if (!(await fileExists(base))) return { status: "not_found", workflowId };
  if (runId) {
    const sp = path.join(base, runId, "state.snapshot.json");
    if (!(await fileExists(sp))) return { status: "not_found", workflowId, runId };
    return { status: "pass", workflowId, runId, state: JSON.parse(await readFile(sp, "utf8")) };
  }
  const runs = (await readdir(base)).sort().reverse();
  if (runs.length === 0) return { status: "no_runs", workflowId };
  const latest = runs[0];
...
export async function workflowHistory({ target, workflowId }) {
  const base = path.join(target, RUNS_DIR, workflowId);
  if (!(await fileExists(base))) return { status: "pass", workflowId, runs: [] };
  const history = [];
...
export async function workflowLogs({ target, workflowId, runId }) {
  const ep = path.join(target, RUNS_DIR, workflowId, runId, "events.jsonl");
  if (!(await fileExists(ep))) return { status: "not_found", workflowId, runId };
...
export async function workflowInstall({ root, target, pluginId }) {
  ...
  if (!plugin) throw new PlatformError(`plugin not found: ${pluginId}`, { code: "AI_ENGINEERING_UNKNOWN_PLUGIN" });

  const assets = plugin.assets?.workflows ?? [];
  if (assets.length === 0) return { status: "pass", pluginId, installed: [] };
