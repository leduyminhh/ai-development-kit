...
function sectionList(markdown, heading) {
  return [...sectionText(markdown, heading).matchAll(/^- (.+)$/gm)].map(
    (item) => item[1].trim(),
  );
}
...
export function validateCanonicalCommand(
  ...
  if (!/^\d+\.\d+\.\d+$/.test(command.version ?? "")) {
    ...
    errors.push(
      `command ${label} version must match plugin version ${command.pluginVersion}`,
    );
  }
  ...
  if (validateReferences) {
    ...
    if (command.mcpTool && !knownMcpTools.has(command.mcpTool)) {
      errors.push(
        `command ${label} references missing MCP tool ${command.mcpTool}`,
      );
    }
  }
  return errors;
...
export async function loadCanonicalCommand(input) {
  ...
  try {
    ...
    throw commandError(
      `missing command ${sourcePath ? path.basename(sourcePath, ".md") : "unknown"}`,
    );
  }
  ...
  for (const heading of REQUIRED_SECTIONS) {
    if (!sectionText(body, heading)) {
      throw commandError(
        `command ${metadata.id ?? sourcePath} has empty ${heading}`,
      );
    }
  }
  const command = {
    ...
    steps: [...sectionText(body, "Steps").matchAll(/^\d+\. (.+)$/gm)].map(
      (item) => item[1].trim(),
    ),
