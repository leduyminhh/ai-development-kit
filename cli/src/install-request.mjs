...
export function parseInstallRequest(args) {
  ...
  return {
    rootPlugins: field(
      [...new Set(positional)].sort(),
      positional.length > 0 || all ? "explicit" : "default",
    ),
    all: field(all, all ? "explicit" : "default"),
    providers: field(providers ?? [], providers ? "explicit" : "default"),
    optionalPlugins: field(
      optionalPlugins ?? [],
      optionalPlugins ? "explicit" : "default",
    ),
...
export function applyDetectedProviders(draft, providers) {
  if (draft.providers.locked) return draft;
  return {
    ...draft,
...
export function finalizeNonInteractiveDraft(draft) {
  ...
  if (
    draft.providers.source !== "explicit" ||
  ...
  if (missing.length > 0) {
    throw argumentError(
      `Missing install choices in non-interactive mode: ${missing.join(", ")}.\n` +
