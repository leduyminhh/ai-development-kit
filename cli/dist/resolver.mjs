...
function isPlatformCompatible(range, version) {
    ...
    return ((!minimum || major >= Number(minimum[1])) &&
...
export function resolvePluginGraph({ requested, plugins, platformVersion, providers, }) {
    ...
    function visit(pluginId) {
        ...
        if (!isPlatformCompatible(plugin.compatibility?.platform, platformVersion)) {
            throw new PlatformError(`plugin ${pluginId} is incompatible with platform ${platformVersion}`, { code: "AI_ENGINEERING_INCOMPATIBLE" });
        }
        for (const provider of providers) {
            if (plugin.compatibility?.providers?.[provider] !== "supported") {
                throw new PlatformError(`plugin ${pluginId} does not support provider ${provider}`, { code: "AI_ENGINEERING_INCOMPATIBLE" });
            }
        }
