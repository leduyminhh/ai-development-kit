...
import { findCommandPath, findSkillPath, loadCanonicalCommand, loadPlatform, loadPlugins, validateRepository, } from "./contracts.mjs";
import { listFiles, replaceDirectoryAtomic, resolveInside, sha256File, writeJsonAtomic, } from "./io.mjs";
...
async function copyDirectory(source, destination) {
    ...
    await cp(source, destination, {
        recursive: true,
        filter: (pathname) => !pathname.includes("__pycache__") && !pathname.endsWith(".pyc"),
    });
}
export async function buildPlugin({ root, pluginId, outputRoot }) {
    ...
    try {
        for (const skill of graph.skills) {
            await copyDirectory(await findSkillPath(root, skill), resolveInside(staged, `skills/${skill}`));
        }
        const commands = [];
        ...
        for (const agent of graph.agents) {
            const source = path.join(root, "adapters", "codex", "agents", `${agent}.toml`);
        ...
        const destination = path.join(outputRoot, pluginId, plugin.metadata.version);
...
export async function buildAllDistributions({ root, outputRoot }) {
    ...
    for (const artifact of pluginArtifacts) {
        distributions.push(await preparePluginDistribution({
