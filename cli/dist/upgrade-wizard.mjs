 ...
 */
export function renderUpgradeStep({ title, updates = [], selected = [], cursor = 0, allowAll = false, all = false, }) {
 ...
 */
export function applyChecklistAction({ action, updates, selected, cursor, all = false, allowAll = false, }) {
    let nextSelected = [...selected];
    let nextAll = all;
    let nextCursor = cursor;
    if (action === "down")
        nextCursor = Math.min(updates.length - 1, cursor + 1);
    if (action === "up")
 ...
 */
export async function runUpgradeWizard({ outdated, prompter, onConfirm = async () => { }, }) {
