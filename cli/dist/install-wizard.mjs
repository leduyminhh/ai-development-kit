...
function pluginDescriptions(availablePlugins) {
    return Object.fromEntries(availablePlugins.map((item) => [item.id, item.description ?? item.metadata?.description ?? ""]));
}
...
function normalizePluginAnswer(answer) {
    if (Array.isArray(answer))
        return { all: false, selected: answer };
...
function optionalCandidates(draft, availablePlugins) {
    ...
    return [
        ...new Set(rootPlugins.flatMap((id) => byId.get(id)?.dependencies?.optional ?? [])),
    ].sort();
}
export async function runInstallWizard({ draft: originalDraft, availablePlugins, detectedProviders, detectedPlugins = [], existingSession = null, onSession = async () => { }, preparePlan, prompter, }) {
    ...
    if (!draft.rootPlugins.locked && !draft.all.locked) {
        ...
        if (isCancelAnswer(rootPluginAnswer))
    ...
    if (!draft.providers.locked) {
        ...
        if (isCancelAnswer(providersAnswer))
            return { action: "cancel" };
    ...
    if (!draft.optionalPlugins.locked && candidates.length > 0) {
        ...
        if (isCancelAnswer(optionalAnswer))
            return { action: "cancel" };
...
export function renderChecklistStep({ title, choices, selected = [], cursor = 0, allowAll = false, all = false, detected = [], descriptions = {}, stepType = "plugins", // plugins, providers, optional, scope
 }) {
...
export function applyChecklistAction({ action, choices, selected, cursor, all = false, allowAll = false, }) {
    let nextSelected = [...selected];
    let nextAll = all;
    let nextCursor = cursor;
    if (action === "down")
        nextCursor = Math.min(choices.length - 1, cursor + 1);
    if (action === "up")
