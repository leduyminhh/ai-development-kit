...
export async function generateRegistry({ root, artifactsRoot, registryRoot }) {
    ...
    for (const [pluginId, plugin] of plugins) {
        ...
        const artifactIntegrity = integrity(await sha256File(path.join(artifactRoot, "checksums.json")));
        const versionEntry = {
            version,
            ...
            github: {
                url: `https://github.com/leduyminhh/ai-engineering-platform/releases/download/` +
...
export async function resolveArtifactSource(entry, fetchSource) {
    ...
    const expected = result.source === entry.github.url ? entry.github.integrity : entry.npm.integrity;
