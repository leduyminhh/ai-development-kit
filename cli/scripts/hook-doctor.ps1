param(
    [string]$Root = $(if (Test-Path (Join-Path $PSScriptRoot '../src/commands/generate-adapter.ts')) {
...
$requiredFiles = @(
    ...
    "$scriptPrefix/hook-service.ps1",
    "$scriptPrefix/install-hooks.ps1"
)
...
$mapped = @("$scriptPrefix/test-hook-core.ps1", "$scriptPrefix/test-hook-service.ps1", "$scriptPrefix/test-install-hooks.ps1") | Where-Object {
