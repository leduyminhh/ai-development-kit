...
- `resources/context-loading-selector.md`: choose which React resource and subagent prompt to load for the current task.
...
2. Load `resources/context-loading-selector.md` first, then load only the matching resource and subagent prompt.
3. Load `resources/frontend-composition-guidelines.md` only when the task has visible UI, layout, visual hierarchy, page design, app surface, or motion.
4. If a design source is present, extract layout, visual hierarchy, tokens, assets, responsive behavior, and interaction intent before coding.
5. If no design source is present, define:
   - visual thesis
   - content plan
   - interaction thesis
   - primary workspace or hero strategy
   - rejection criteria for generic UI
6. Inspect existing React stack before editing:
...
7. Implement within existing project conventions. Prefer focused edits over rewrites.
8. Translate API examples into a small client layer or existing data-fetching pattern:
...
9. Verify UI and behavior with the project's available commands.
10. Prepare completion notes that can later be sent to Notion or Slack automation.
