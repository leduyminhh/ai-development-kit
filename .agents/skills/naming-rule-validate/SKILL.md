...
`e2e`, `edge`, `er`, `feature`, `figma`, `fixture`, `flaky`, `flow`, `form`, `gantt`,
`grammar`, `handoff`, `ie`, `infrastructure`, `json`, `merge`, `mindmap`, `module`,
...
Selected tests automatically run naming validation for changes under `.codex/agents`, `.agents/skills`, `.codex/hooks`, and `scripts`.
