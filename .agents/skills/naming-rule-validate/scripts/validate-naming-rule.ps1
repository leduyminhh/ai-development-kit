...
function Test-CapabilityName {
    ...
    $qualifiers = @($domains + @(
        ...
        'deployment', 'domain', 'e2e', 'edge', 'er', 'feature', 'figma', 'fixture',
        'flaky', 'flow', 'form', 'gantt', 'grammar', 'handoff', 'ie', 'infrastructure',
        'json', 'merge', 'mindmap', 'module', 'network', 'object', 'output', 'path',
...
function Test-ScriptName {
    ...
    $scriptVerbs = @(
        ...
        'run', 'test', 'install', 'resolve', 'invoke'
    )
...
function Get-NameFromPath {
    ...
    if ($relativePath -match '^\.agents/skills/[^/]+/scripts/([^/]+)\.ps1$') {
...
function Get-TargetFiles {
    ...
    foreach ($searchRoot in @('.codex/agents', '.codex/hooks', '.agents/skills', 'scripts')) {
        $fullSearchRoot = Join-Path $Root $searchRoot
        if (Test-Path -LiteralPath $fullSearchRoot) {
            Get-ChildItem -LiteralPath $fullSearchRoot -Recurse -File -Include '*.toml', '*.md', '*.ps1' -ErrorAction SilentlyContinue
        }
    }
}
