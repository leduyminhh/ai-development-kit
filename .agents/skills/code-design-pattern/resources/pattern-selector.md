...
3. `pattern-creation-generate`: highest priority for constructor complexity, object families, optional fields, immutable setup, or controlled lifecycle.
4. `pattern-architecture-design`: highest priority for application-level boundaries, web entry flow, distributed transactions, DTO boundaries, gateway/filter/page-object concerns.
