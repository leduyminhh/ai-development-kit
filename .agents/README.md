# Agents Runtime Assets

Thu muc `.agents/` chua runtime skills cua repo. Moi skill la mot don vi co the duoc Codex kich hoat khi user request phu hop.
...
| `agents/openai.yaml` | Khuyen nghi | Metadata hien thi skill trong UI. |
...
- Chay `scripts/test-readme-skill-catalog.ps1` de bat truong hop README chua duoc cap nhat.
...
4. Cap nhat `.codex/config.toml` agent registration hoac chay validator voi `-Fix` de sync.
5. Neu co script test, map vao `.codex/test-map.toml`.
6. Chay validator va selected tests.
