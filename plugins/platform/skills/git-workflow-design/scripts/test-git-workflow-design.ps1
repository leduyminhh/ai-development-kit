...
Assert-FileContainsRegex -Path $bodyRules -Pattern 'Long Body Grouping' 'Commit body rules should define nested grouping for long bodies.'
Assert-FileContainsRegex -Path $bodyRules -Pattern '(?s)large.*breaking.*3 or more main bullets' 'Commit body rules should define when a body is long enough to require grouping.'
Assert-FileContainsRegex -Path $bodyRules -Pattern 'summary.*supporting facts' 'Commit body rules should separate parent summaries from supporting facts.'
...
Assert-FileContainsRegex -Path $templates -Pattern 'Long Body Template' 'Commit templates should include a nested long-body example.'
...
Assert-FileContainsRegex -Path $skill -Pattern 'long body.*nested detail bullets' 'SKILL.md should require nested detail bullets for long bodies.'
...
Assert-FileContainsRegex -Path $examples -Pattern 'feat\(databases\): add PostgreSQL per-db backup layout' 'Commit convention examples should demonstrate a long body with grouped database changes.'
