...
test("generates sorted npm and GitHub registry entries", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ["architecture", "backend", "documentation", "frontend", "security", "testing"],
    );
    const backend = JSON.parse(
      await readFile(path.join(temp, "registry", "plugins", "backend.json"), "utf8"),
    );
    const entry = backend.versions[0];
