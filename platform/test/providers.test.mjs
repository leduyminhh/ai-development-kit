...
import { loadCanonicalCommand } from "../src/contracts.mjs";
import {
  projectClaude,
  projectCodex,
  projectCursor,
...
test("projects canonical command semantics for all providers", async () => {
  const command = await loadCanonicalCommand(
    path.join(repoRoot, "packages", "backend", "commands", "review-backend.md"),
  );
  const context = {
...
test("emits only contained relative provider paths", async () => {
  ...
  assert.deepEqual(Object.keys(outputs).sort(), ["claude", "codex", "cursor"]);
